...
void executeTest(Test_Case * tc)
{
    ...
    while(tel->type != END)
    {
        ...
        else if (TEL_TX == tel->type)
        {
            ...
            snprintf ( msg
                     , 1024
                     , "%s: Number of bytes in send telegram %d expected %d, sent %d"
                     , tc->name, tn, tel->length, bus.sendTelegramLen - 1
                     );
            INFO(msg);
            ...
            {
                snprintf(temp, 255, " %2d", i + 1);                   strcat(numbers, temp);
                snprintf(temp, 255, " %02x", bus.sendCurTelegram[i]); strcat(received, temp);
                snprintf(temp, 255, " %02x", tel->bytes[i]);          strcat(expected, temp);
                if (tel->bytes[i] != bus.sendCurTelegram[i])
                {
                    c++;
                    snprintf (temp, 1024, "%d, ", i + 1);             strcat(msg, temp);
                }
            }
