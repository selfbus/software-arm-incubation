/* Origin: cdc_vcom.c from nxp_lpcxpresso_11u14_usbd_lib_cdc example
...
// ***NEW NEW NEW***
/* Set control line state call back routine */
static ErrorCode_t VCOM_SetCtrlLineState(USBD_HANDLE_T hCDC, uint16_t state)
{
  //called on SET_CONTROL_LINE_STATE
    //state & 2 -> RTS     ISP_EN
    //state & 1 -> DTR     ISP_RESET
    VCOM_DATA_T *pVcom = &g_vCOM;
    pVcom->vcom_ctrllines = ~(state & 0xff);
    return LPC_OK;
}
...
uint32_t vcom_read_cnt(void)
{
    ...
    {
        ret = pVcom->rx_count;
        //pVcom->rx_count = 0; commented out
    }
