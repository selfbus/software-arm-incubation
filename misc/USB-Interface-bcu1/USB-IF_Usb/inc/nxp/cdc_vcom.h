...
typedef struct VCOM_DATA {
	...
	volatile uint8_t vcom_ctrllines;
} VCOM_DATA_T;
...
/**
 * @brief Return the state of the control lines
 * @return Bit 0: DTR (set -> TTL high -> RS232 negative)
 *         Bit 1: RTS (set -> TTL high -> RS232 negative)
 */
static INLINE uint8_t vcom_readctrllines(void) {
	return g_vCOM.vcom_ctrllines;
}
